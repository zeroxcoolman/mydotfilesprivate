...
apply_rofi_theme_to_config() {
  ...
  local theme_path="$ROFI_THEMES_DIR/$theme_name_to_apply"
  ...
  # Comment out any existing @theme entry (fixed sed command)
  ...
  if [ "$total_lines" -gt "$max_lines" ]; then
    ...
    for ((i = 1; i <= excess; i++)); do
      sed -i '0,/^\/\/\s*@theme/d' "$ROFI_CONFIG_FILE"
    done
  fi
...
if [ ! -d "$ROFI_THEMES_DIR" ]; then
  notify_user "$IDIR/error.png" "E-R-R-O-R" "Rofi themes directory not found: $ROFI_THEMES_DIR"
  exit 1
fi
...
mapfile -t available_theme_names < <(find "$ROFI_THEMES_DIR" -maxdepth 1 -name "*.rasi" -type f -printf "%f\n" 2>/dev/null | sort -V)

if [ ${#available_theme_names[@]} -eq 0 ]; then
  notify_user "$IDIR/error.png" "No Rofi Themes" "No .rasi files found in $ROFI_THEMES_DIR"
  exit 1
fi
...
while true; do
  ...
  # Launch Rofi and get user's choice (fixed &amp; escaping)
