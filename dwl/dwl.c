...
swapstack(const Arg *arg)
{
    ...
    /* Find next/previous client on the same monitor */
    if (arg->i > 0) {
        n = wl_container_of(c->link.next, n, link);
        while (&n->link != &clients && n->mon != selmon)
            n = wl_container_of(n->link.next, n, link);
        if (&n->link == &clients)
            return;
    } else {
        n = wl_container_of(c->link.prev, n, link);
        while (&n->link != &clients && n->mon != selmon)
            n = wl_container_of(n->link.prev, n, link);
        if (&n->link == &clients)
            return;
    }

    /* Swap positions in the tiling list */
    wl_list_remove(&c->link);
    wl_list_insert(arg->i > 0 ? &n->link : n->link.prev, &c->link);
